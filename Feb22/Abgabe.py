...
def collectData(filename):
    with open(
        os.path.join(os.path.dirname(__file__), "input_data", filename), "r"
    ) as f:

        # read contributor and project count
        contributor_count, project_count = [
        ...
        project_data = pd.DataFrame(
            columns=["name", "days", "score", "bestbefore", "F", "done"]
        )
        ...
        for _ in range(contributor_count):
            ...
            for _ in range(int(skill_count)):
                ...
                contributor_data.loc[len(contributor_data)] = [
                    contributor_name,
                    skill_name,
                    int(skill_level),
                ]
        ...
        for _ in range(project_count):
            (
                ...
                project_score,
                project_bestbefore,
                project_reqskills_count,
            ) = (
                f.readline().strip().split()
            )
            project_data.loc[len(project_data)] = [
                project_name,
                int(project_days),
                int(project_score),
                int(project_bestbefore),
                0,
                0,
            ]
            for _ in range(int(project_reqskills_count)):
                skill_name, skill_level = f.readline().strip().split()
                project_req.loc[len(project_req)] = [
                    project_name,
                    skill_name,
                    int(skill_level),
                ]
...
def give_solution_pls(
    contributor_data, contributor_occ, project_data, project_req
) -> list:
    ...
    while (everything_ready(project_data)) or day == 0:
        ...
        for _, project in project_data.iterrows():
            if project["done"] == 1:
                ...
                if project["days"] <= 0:
                    if project["days"] < 0:
                        print("Math rong")
                    project_data.loc[
                        project_data["name"] == project["name"], "done"
                    ] = 2
                continue
            ...
            for req in requirements:
                ...
                possible_contr = contributor_data[
                    (contributor_data["skill"] == req[0])
                    & (contributor_data["level"] >= req[1])
                ]

                possible_contr = possible_contr.sort_values(
                ...
                for _, contr in possible_contr.iterrows():
                    ...
                    if (
                        contr["name"]
                        in occ_copy.loc[(occ_copy["occupied"] < day)].values
                    ):
                        #!!!Potential error
                        occ_copy.loc[occ_copy["name"] == contr["name"], "occupied"] = (
                            day + int(project["days"]) - 1
                        )
